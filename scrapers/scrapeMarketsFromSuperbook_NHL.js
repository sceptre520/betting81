      ...
      const marketGroups = Promise.resolve(scrapedOdds).then((scrapedOdds) => {
        ...
        if (scrapedOdds.ismatch && scrapedOdds.istradable) {
          ...
          if (filteredEvents) {
            ...
            let stats = ["Goals", "Points", "Assists", "Shots"]; //I have no idea what is the naming convention
            for (const stat of stats) {
              const playerMarkets = markets.filter((o) => {
                return (
                  o.name.endsWith(` ${stat}`) &&
                  !o.name.includes("+") &&
                  !o.name.includes("Powerplay")
                ); //avoid the P+R+A-type markets
              });
