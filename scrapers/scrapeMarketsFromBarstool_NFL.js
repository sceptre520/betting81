...
deleteOneSportsbookMarketsFromDB("Barstool");
...
const getBarstoolOdds = (URL_for_odds) => {
...
Promise.resolve(matchList)
  ...
  .then((matchList) => {
    matchList.map((BVmatch) => {
      ...
      const BV_Markets_URL = `https://eu-offering.kambicdn.org/offering/v2018/pivusco/betoffer/event/${BVEventId}.json?lang=en_US&market=US-CO&includeParticipants=true`;

      //Promise API call
      const scrapedOdds = getBarstoolOdds(BV_Markets_URL).then((data) => {
        return data;
      });
