...
Promise.resolve(matchList)
  ...
  .then((matchList) => {
    matchList.map((BVmatch) => {
      ...
      const BV_Markets_URL = `https://eu-offering.kambicdn.org/offering/v2018/888usnj/betoffer/event/${BVEventId}.json?lang=en_US&market=US-CO&includeParticipants=true`;

      //Promise API call
      const scrapedOdds = get888Odds(BV_Markets_URL).then((data) => {
        return data;
      });
