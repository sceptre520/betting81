...
exports.scrapeWHMarkets = async () => {
  return deleteOneSportsbookMarketsFromDB("William Hill").then(() => {
    ...
    return Promise.resolve(matchList)
      ...
      .then((matchList) => {
        matchList.map((WHmatch) => {
          ...
          //Promise API call
          const scrapedOdds = getWilliamHillOdds(WH_Markets_URL).then(
            (data) => {
              return data;
            }
          );
                  ...
                  pointsMarkets.map((thisPointsMarket) => {
                    Outcomes = thisPointsMarket.selections.map((outcome) => {
                      if (outcome.display && outcome.active) {
                        const nameWithoutPipes = outcome.name.replace(
                          /\|/g,
                          ""
                        );
