  ...
  (values) => {
    ...
    for (let i = 0; i < PBEventMapping.length; i++) {
      ...
      let matchFromDB = matchList.filter((o) => {
        return o.name === thisPBEvent.name;
      });
          ...
          return (
            (((o.groupName === "Player Points Markets" &&
              ...
              (o.groupName === "Player Rebounds Markets" &&
                o.eventClass.includes(" Rebounds Over/Under"))) &&
              (o.eventClass.includes("Home") ||
                o.eventClass.includes("Away"))) ||
            o.eventClass === "First Basket"
          );
        });
    ...
    Promise.all([scrapedOdds, UniqueEventClasses, PBEvent]).then((data) => {
      ...
      if (UniqueEventClasses.length) {
        let Mydata = UniqueEventClasses.map((eventClass) => {
          ...
          let marketType;
          if (eventClass.includes("Points")) {
            marketType = "Points";
          } else if (eventClass.includes("Assists")) {
            marketType = "Assists";
          } else if (eventClass.includes("Rebounds")) {
            marketType = "Rebounds";
          } else if (eventClass.includes("First Basket")) {
            marketType = "First Basket";
          } else {
          ...
          if (GiannisMarket) {
            ...
            if (Array.isArray(marketOutput)) {
              //console.log("I am array");
              marketOutput.map((market) => {
                createMarketInDB(market);
              });
            } else {
              //console.log("I am not array");
              createMarketInDB(marketOutput);
            }
          }
        });
      }
